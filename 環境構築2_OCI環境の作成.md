...
![alt text](<img/スクリーンショット 2026-02-25 213825.png>)
...
作製した仮想クラウドネットワークの画面にて「セキュリティ」タブ → 「セキュリティ・リスト」欄から、デフォルトで存在するセキュリティ・リストを選択する。<br>

![alt text](</img/スクリーンショット 2026-02-13 222123.png>)<br>

<br>

「セキュリティルール」タブから、イングレス・ルールにデフォルトで設定されている  `0.0.0.0/0` のTCP許可設定を編集する。<br>
...
作製した仮想クラウドネットワークの画面にて、「セキュリティ」タブ → 「ネットワーク・セキュリティ・グループの作成」を押下する。<br>
...
### インターネットへのパブリック・サブネットの接続
作成したインスタンスの「ネットワーキング」タブにて、インターネットへのパブリックサブネットの接続から、「接続」を押下する

> ℹ️ これにより、パブリックサブネットがインターネットに接続され、インターネットからRDPができるようになります。

![alt text](</img/スクリーンショット 2026-02-25 211946.png>)<br>

<br>

デフォルト値のまま、作成する

![alt text](</img/スクリーンショット 2026-02-25 212244.png>)<br>

<br>


作成したインスタンスに対して、パブリックIP・ユーザー名・初期パスワードを使用してローカルPCからRDP接続できることを確認する。<br>
...
![alt text](</img/スクリーンショット 2026-02-25 195144.png>)

<br>

作成したDRGの「アタッチメント」タブで、「仮想クラウド・ネットワーク・アタッチメントの作成」を押下する。<br>
...
作製した仮想クラウドネットワークの画面にて、「ルーティング」タブ → デフォルトのルート表を選択する。<br>
...
Azure側のIPアドレス範囲宛の通信が、  作成したDRGをネクストホップとするよう設定して作成する。

| 項目 | 入力方法 |
|:---------|:---------|
| ターゲット・タイプ | 動的ルーティング・ゲートウェイ |
| 宛先タイプ | CIDRブロック |
| 宛先CIDRブロック | 宛先となるAzure環境の仮想ネットワークのIP範囲 |
| ターゲット動的ルーティング・ゲートウェイ | 作製したDRG |
